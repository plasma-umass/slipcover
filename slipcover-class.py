...
def instrument(co):
    ...
    if (isinstance(co, types.FunctionType) or isinstance(co, types.MethodType)):
...
def all_functions():
    ...
    # (and filter out our own!) ... or use wrappers?
    classes = [c[1] for c in inspect.getmembers(sys.modules[__name__], inspect.isclass)]
    return [f[1] for c in classes for f in inspect.getmembers(c, inspect.isfunction)]
